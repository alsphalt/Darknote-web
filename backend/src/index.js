...
export async function createServer() {
  ...
  const io = new Server(server, {
    ...
    // Vercel (Fluid compute) only supports WebSocket transport — Socket.IO
    // HTTP long-polling does not work on serverless functions. Locally this
    // also works (modern browsers all support WebSocket).
...
// Vercel's zero-config Express detection (docs, Aug 2026) requires a file at
// app/index/server (root or src/) that DEFAULT-EXPORTS the Express app or the
// http server. We export the built http server (Socket.IO attached).
// Building it at import time is safe: no listen happens here, and Prisma only
// connects lazily on the first query. Local dev (`node src/index.js`) takes
// the isMain branch above and starts its own listener.
const vercelServer = await createServer();
export default vercelServer.server;
