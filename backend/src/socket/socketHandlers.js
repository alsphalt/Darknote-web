  ...
  try {
    if (state.phase === GAME_PHASE.SELECT) {
      ...
      if (legal.length) {
        ...
        // Serverless note: persist before broadcast so a recycled instance
        // (Vercel Fluid) resumes from this exact state on reconnect.
...
async function handleRoll(io, roomId, { auto = false } = {}) {
  ...
  // No legal move with this roll → turn advanced (or re-roll chained) above.
  gameManager.set(roomId, state);
  // Serverless note: persist the post-roll turn/phase before broadcasting.
  await persistGameState(roomId, state);
  io.to(roomId).emit('game:state', state);
  scheduleTurnTimer(io, roomId);
  return { ok: true, legal: [], auto };
